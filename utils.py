...
def get_kitti_attributes():
  ...
  categories = [{"id": key, "name": value, "color": color_map[key]} for (key, value) in labels.items()]
...
def label_kitti_to_segments(label):
  ...
  for cat in unique_cats:
    ...
    for i, instances_cat in enumerate(instances_cats):
      if instances_cat == cat and unique_instances[i] > 0:
        annotations.append({
          "id": int(unique_instances[i]),
          "category_id": int(instances_cat)
        })
  ...
  point_annotations = [cat_map[category_ids[i]] if id == 0 else int(id) for i, id in enumerate(instance_ids)]
